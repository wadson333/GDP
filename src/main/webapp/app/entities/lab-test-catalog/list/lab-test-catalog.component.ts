...
export class LabTestCatalogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.labTestCatalogs || this.labTestCatalogs.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.labTestCatalogs = this.refineData(dataFromBody);
  }

  protected refineData(data: ILabTestCatalog[]): ILabTestCatalog[] {
